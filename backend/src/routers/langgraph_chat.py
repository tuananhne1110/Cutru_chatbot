...
from src.langgraph_rag.tracking.langfuse_tracing import LANGFUSE, observe
...
async def langgraph_chat(request: ChatRequest) -> ChatResponse:
    ...
    with LANGFUSE.start_as_current_span(name="workflow.invoke") as span:
        span.update(
            metadata= {
                "session_id": session_id,
                "msg_count": len(initial_state["conversation_history"]),
            }
        )
        span.update(input={
            "question": request.question,
        ...
        span.update(output={"answer_preview": answer[:200]})
