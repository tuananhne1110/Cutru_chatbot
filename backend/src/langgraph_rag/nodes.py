...
from .tracking.langfuse_tracing import LANGFUSE, observe
...
class RAGWorkflowNodes:
    ...
    @observe(name="node.input_validation")
    def input_validation_node(self, state: RagState) -> RagState:
        logger.info("--- NODE: INPUT VALIDATION & GUARDRAIL ---")
        quesion = state["question"]
        try:
            if LANGFUSE:
                with LANGFUSE.start_as_current_span(name="guardrails.inut") as sp:
                    sp.update(
                        input= {
                            "text_preview": _preview_text(quesion),
                        },
                        metadata= {
                            "source_type": "INPUT"
                        },
                    ...
                    sp.update(output={"action": result.get("action", "NONE")})
    ...
    @observe(name="node.query_analysis")
    def query_analysis_node(self, state: RagState) -> RagState:
        ...
        if LANGFUSE:
            with LANGFUSE.start_as_current_span(name="query.route") as sp:
                sp.update(input={"question_preview": _preview_text(quesion)})
                intents, total_token = self.query_route.query_route(quesion)
                sp.update(output={"intents": intents})
        else:
            intents, total_token = self.query_route.query_route(quesion)
    ...
    @observe(name="node.document_retrieval")
    def document_retrieval_node(self, state: RagState) -> RagState:
        ...
        try:
            ...
            if LANGFUSE:
                with LANGFUSE.start_as_current_span(name="vector.retrieve") as sp:
                    sp.update(
                        input={"query_preview": _preview_text(quesion)},
                    ...
                    for collection_name in collections:
                        ...
                        if "question" in docs[0]['payload'].keys() and "answer" in docs[0]['payload'].keys():
                            break
                    sp.update(output={"num_docs": len(all_documents)})
    ...
    @observe(name="node.answer_generation")
        ...
        try:
            ...
            if LANGFUSE:
                if state["current_status"] == "GENERAL_QUERY":
                    with LANGFUSE.start_as_current_span(name="answer.generate") as sp:
                        sp.update(
                            metadata={"mode": "general"},
                            input={
                                "question_preview": _preview_text(quesion),
                                "history_preview": _preview_text(conversation_history)
                            }
                        )
                        ...
                        sp.update(output={"answer_preview": _preview_text(generated_answer)})
                else:
                    ...
                    with LANGFUSE.start_as_current_span(name="answer.generate") as sp:
                        sp.update(
                            ...
                            input={
                                "question_preview": _preview_text(quesion),
                                "context_preview": _preview_text(relevant_context),
                                "history_preview": _preview_text(conversation_history)
                            }
                        )
    ...
    @observe(name="node.output_validation")
    def output_validation_node(self, state: RagState) -> RagState:
        ...
        try:
            ...
            if LANGFUSE:
                with LANGFUSE.start_as_current_span(name="guardrails.output") as sp:
                    sp.update(
                        input={"answer_preview": _preview_text(answer)},
                        metadata={"source_type": "OUTPUT"}
                    )
                    result = self.bedrock_guardrails.apply_guardrail(
                        text=answer,
                        source_type="OUTPUT"
