...
async def retrieve_context(state: ChatState) -> ChatState:
    ...
    for collection in collections:
        try:
            ...
            if len(collections) == 1:
                search_limit = 10  # Nếu chỉ 1 collection thì lấy nhiều hơn
            elif len(collections) == 2:
                search_limit = 10   # Nếu 2 collections thì lấy ít hơn mỗi collection
            else:
                search_limit = 10   # Nếu nhiều collections thì lấy ít nhất
    ...
    if all_docs:
        ...
        filtered_docs = [doc for doc in all_docs if doc.metadata.get("rerank_score", 0.0) >= 0.0]
