...
Usage:
    from utils.voice_processing.vietnamese_config import apply_vietnamese_optimizations
...
VIETNAMESE_STT_CONFIG = {
    ...
    "VOICE_SILENCE_THRESHOLD": "1.5",
    "VOICE_SENTENCE_DETECTION": "true",
...
# Enhanced turn detection settings for automatic voice chat
def get_auto_voice_config():
    """
    Get configuration specifically for automatic voice chat.
    
    Returns:
        dict: Configuration for auto voice chat with turn detection
    """
    config = get_vietnamese_config()
    
    # Override some settings for better auto voice chat experience
    config.update({
        "VOICE_TURN_DETECTION": "true",
        "VOICE_SILENCE_THRESHOLD": "1.2",  # Slightly shorter for responsiveness
        "VOICE_SENTENCE_DETECTION": "true",
        "STT_POST_SILENCE": "0.6",  # Faster response
        "STT_REALTIME_PAUSE": "0.005",  # More responsive
    })
