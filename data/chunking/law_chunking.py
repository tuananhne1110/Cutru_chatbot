...
def extract_law_meta(raw_text: str):
    ...
    for i, line in enumerate(lines[:20]):
        ...
        if re.search(r"(Căn cứ|Nghị định|Điều|Quy định)", line, re.IGNORECASE):
            continue
        m = re.search(r"(Luật số|Số)[:：]?\s*([0-9]{1,4}/[0-9]{4}/[^\s,.;:]+)", line, re.IGNORECASE)
        if m:
...
# ==== MAIN: XỬ LÝ FOLDER ====
FOLDER = "data/laws"  # Đổi thành thư mục chứa các file .docx
os.makedirs("data/chunking/output_json", exist_ok=True)
...
out_path = os.path.join("data/chunking/output_json", "all_laws.json")
